...
static uint8_t bar_color_idx(lv_color_t c) {
    lv_color_t p[] = {THEME_COLOR_BLUE_DARK, THEME_COLOR_RED, THEME_COLOR_GREEN_BRIGHT,
        THEME_COLOR_YELLOW, THEME_COLOR_ORANGE, THEME_COLOR_PURPLE,
        THEME_COLOR_CYAN, THEME_COLOR_MAGENTA};
    for (int i = 0; i < 8; i++) if (c.full == p[i].full) return i;
    return 8;
}

static uint8_t rpm_color_idx(lv_color_t c) {
    lv_color_t p[] = {THEME_COLOR_GREEN, THEME_COLOR_CYAN, THEME_COLOR_YELLOW,
        THEME_COLOR_ORANGE, THEME_COLOR_RED, THEME_COLOR_BLUE,
        THEME_COLOR_PURPLE, THEME_COLOR_MAGENTA, THEME_COLOR_PINK};
    for (int i = 0; i < 9; i++) if (c.full == p[i].full) return i;
    return 9;
}
...
static uint8_t *id_alloc(uint8_t v) {
    uint8_t *p = lv_mem_alloc(sizeof(uint8_t)); *p = v; return p;
}
...
static void build_can_tab(lv_obj_t *tab, uint8_t value_id)
{
    ...
    settings_section_t *sec = settings_add_section(tab, "CAN BUS SIGNAL", THEME_COLOR_ACCENT);

    char can_id_str[16];
    snprintf(can_id_str, sizeof(can_id_str), "%X", values_config[idx].can_id);
    g_can_id_input[idx] = settings_add_text_input(sec, "CAN ID (0x):", "hex", can_id_str);
    ...
    g_endian_dropdown[idx] = settings_add_dropdown(sec, "Endian:", "Big Endian\nLittle Endian", 0);
    ...
    g_scale_input[idx] = settings_add_text_input(sec, "Scale:", "factor", scale_str);
    ...
    g_type_dropdown[idx] = settings_add_dropdown(sec, "Data Type:", "Unsigned\nSigned", 0);
...
{
    uint8_t idx = value_id - 1;
    settings_section_t *sec = settings_add_section(tab, "DISPLAY OPTIONS", THEME_COLOR_ACCENT_TEAL);

    g_label_input[idx] = settings_add_text_input(sec, "Label:", "panel label", label_texts[idx]);
    ...
    lv_obj_t *du = settings_add_text_input(sec, "Display Unit:", "suffix", values_config[idx].custom_text);
    lv_obj_add_event_cb(du, keyboard_event_cb, LV_EVENT_ALL, NULL);
    { uint8_t *p = id_alloc(value_id);
      lv_obj_add_event_cb(du, custom_text_input_event_cb, LV_EVENT_VALUE_CHANGED, p);
...
static void build_display_tab_bar(lv_obj_t *tab, uint8_t value_id)
{
    ...
    g_label_input[idx] = settings_add_text_input(sec, "Label:", "bar label", label_texts[idx]);
    lv_obj_add_event_cb(g_label_input[idx], keyboard_event_cb, LV_EVENT_ALL, NULL);
    { uint8_t *p = id_alloc(value_id);
      lv_obj_add_event_cb(g_label_input[idx], label_input_event_cb, LV_EVENT_VALUE_CHANGED, p);
      lv_obj_add_event_cb(g_label_input[idx], free_value_id_event_cb, LV_EVENT_DELETE, p); }
    ...
    { uint8_t *p = id_alloc(value_id);
      lv_obj_add_event_cb(bmin, bar_range_input_event_cb, LV_EVENT_VALUE_CHANGED, p);
      lv_obj_add_event_cb(bmax, bar_range_input_event_cb, LV_EVENT_VALUE_CHANGED, p); }

    lv_obj_t *sv = settings_add_switch(sec, "Show Value:", values_config[idx].show_bar_value);
    { uint8_t *p = id_alloc(value_id);
      lv_obj_add_event_cb(sv, show_value_switch_event_cb, LV_EVENT_VALUE_CHANGED, p);
      lv_obj_add_event_cb(sv, free_value_id_event_cb, LV_EVENT_DELETE, p); }

    lv_obj_t *iv = settings_add_switch(sec, "Invert Value:", values_config[idx].invert_bar_value);
    { uint8_t *p = id_alloc(value_id);
      lv_obj_add_event_cb(iv, invert_value_switch_event_cb, LV_EVENT_VALUE_CHANGED, p);
    ...
    if (!fs_on) lv_obj_add_flag(fs_box, LV_OBJ_FLAG_HIDDEN);
    ...
    lv_obj_t *fs_ei = settings_add_text_input(fs_sec, "Empty Voltage:", "V", vb);
    lv_obj_add_event_cb(fs_ei, keyboard_event_cb, LV_EVENT_ALL, NULL);
    { uint8_t *p = id_alloc(value_id);
      lv_obj_add_event_cb(fs_ei, fs_empty_v_input_event_cb, LV_EVENT_VALUE_CHANGED, p);
    ...
    { uint8_t *p = id_alloc(value_id);
      lv_obj_add_event_cb(fs_fi, fs_full_v_input_event_cb, LV_EVENT_VALUE_CHANGED, p);
    ...
    char fb[20]; snprintf(fb, sizeof(fb), "Filter: %d%%", values_config[idx].fuel_sender_filter);
    ...
    lv_slider_set_value(fs_sl, values_config[idx].fuel_sender_filter, LV_ANIM_OFF);
    lv_obj_set_style_bg_color(fs_sl, THEME_COLOR_ACCENT, LV_PART_INDICATOR | LV_STATE_DEFAULT);
    lv_obj_set_style_bg_color(fs_sl, THEME_COLOR_BORDER, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_bg_color(fs_sl, THEME_COLOR_ACCENT, LV_PART_KNOB | LV_STATE_DEFAULT);
    ...
    lv_obj_add_event_cb(fs_sl,  fs_filter_slider_event_cb, LV_EVENT_VALUE_CHANGED, ctx);
    lv_obj_add_event_cb(fs_sl,  fs_filter_slider_event_cb, LV_EVENT_RELEASED,      ctx);
    lv_obj_add_event_cb(fss,    fuel_sender_switch_event_cb,   LV_EVENT_VALUE_CHANGED, ctx);
    lv_obj_add_event_cb(fss,    fuel_sender_ctx_free_event_cb, LV_EVENT_DELETE,        ctx);
...
static void build_alerts_tab_panel(lv_obj_t *tab, uint8_t value_id)
{
    ...
    settings_section_t *sec = settings_add_section(tab, "WARNING SETTINGS", THEME_COLOR_ACCENT_AMBER);
    ...
    if (!has_warnings) lv_obj_add_flag(warn_box, LV_OBJ_FLAG_HIDDEN);
    ...
    char buf[20];
    snprintf(buf, sizeof(buf), "%.2f", values_config[idx].warning_high_threshold);
    lv_obj_t *wh = settings_add_text_input(ws, "Range High:", "value", buf);
    lv_obj_add_event_cb(wh, keyboard_event_cb, LV_EVENT_ALL, NULL);
    { uint8_t *p = id_alloc(value_id);
      lv_obj_add_event_cb(wh, warning_high_threshold_event_cb, LV_EVENT_VALUE_CHANGED, p);
      lv_obj_add_event_cb(wh, free_value_id_event_cb, LV_EVENT_DELETE, p); }

    lv_obj_t *whc = settings_add_dropdown(ws, "High Colour:", "Red\nBlue", 0);
    lv_dropdown_set_selected(whc, values_config[idx].warning_high_color.full == THEME_COLOR_RED.full ? 0 : 1);
    { uint8_t *p = id_alloc(value_id);
      lv_obj_add_event_cb(whc, warning_high_color_event_cb, LV_EVENT_VALUE_CHANGED, p);
      lv_obj_add_event_cb(whc, free_value_id_event_cb, LV_EVENT_DELETE, p); }

    snprintf(buf, sizeof(buf), "%.2f", values_config[idx].warning_low_threshold);
    lv_obj_t *wl = settings_add_text_input(ws, "Range Low:", "value", buf);
    lv_obj_add_event_cb(wl, keyboard_event_cb, LV_EVENT_ALL, NULL);
    { uint8_t *p = id_alloc(value_id);
      lv_obj_add_event_cb(wl, warning_low_threshold_event_cb, LV_EVENT_VALUE_CHANGED, p);
      lv_obj_add_event_cb(wl, free_value_id_event_cb, LV_EVENT_DELETE, p); }

    lv_obj_t *wlc = settings_add_dropdown(ws, "Low Colour:", "Red\nBlue", 0);
    lv_dropdown_set_selected(wlc, values_config[idx].warning_low_color.full == THEME_COLOR_RED.full ? 0 : 1);
    { uint8_t *p = id_alloc(value_id);
      lv_obj_add_event_cb(wlc, warning_low_color_event_cb, LV_EVENT_VALUE_CHANGED, p);
...
{
    uint8_t idx = value_id - 1;
    bool has_warnings = (values_config[idx].bar_low  != 0 ||
                         values_config[idx].bar_high != 0);

    settings_section_t *sec = settings_add_section(tab, "BAR THRESHOLDS", THEME_COLOR_ACCENT_AMBER);
    ...
    lv_obj_t *blow = settings_add_text_input(ws, "Low Threshold:", "value", buf);
    lv_obj_add_event_cb(blow, keyboard_event_cb, LV_EVENT_ALL, NULL);
    { uint8_t *p = id_alloc(value_id);
      lv_obj_add_event_cb(blow, bar_low_value_event_cb, LV_EVENT_VALUE_CHANGED, p);
      lv_obj_add_event_cb(blow, free_value_id_event_cb, LV_EVENT_DELETE, p); }

    snprintf(buf, sizeof(buf), "%d", values_config[idx].bar_high);
    lv_obj_t *bhigh = settings_add_text_input(ws, "High Threshold:", "value", buf);
    lv_obj_add_event_cb(bhigh, keyboard_event_cb, LV_EVENT_ALL, NULL);
    { uint8_t *p = id_alloc(value_id);
      lv_obj_add_event_cb(bhigh, bar_high_value_event_cb, LV_EVENT_VALUE_CHANGED, p);
    ...
    lv_dropdown_set_selected(blc, bar_color_idx(values_config[idx].bar_low_color));
    { uint8_t *p = id_alloc(value_id);
      lv_obj_add_event_cb(blc, bar_low_color_event_cb, LV_EVENT_VALUE_CHANGED, p);
      lv_obj_add_event_cb(blc, free_value_id_event_cb, LV_EVENT_DELETE, p); }

    lv_obj_t *bhc = settings_add_dropdown(ws, "High Colour:", BAR_COLOR_OPTS, 0);
    lv_dropdown_set_selected(bhc, bar_color_idx(values_config[idx].bar_high_color));
    { uint8_t *p = id_alloc(value_id);
      lv_obj_add_event_cb(bhc, bar_high_color_event_cb, LV_EVENT_VALUE_CHANGED, p);
      lv_obj_add_event_cb(bhc, free_value_id_event_cb, LV_EVENT_DELETE, p); }

    lv_obj_t *brc = settings_add_dropdown(ws, "In-Range Colour:", BAR_COLOR_OPTS, 0);
    lv_dropdown_set_selected(brc, bar_color_idx(values_config[idx].bar_in_range_color));
    { uint8_t *p = id_alloc(value_id);
      lv_obj_add_event_cb(brc, bar_in_range_color_event_cb, LV_EVENT_VALUE_CHANGED, p);
...
static void build_display_tab_rpm(lv_obj_t *tab, uint8_t value_id)
{
    ...
    /* ── Gauge & Colour ── */
    settings_section_t *sec = settings_add_section(tab, "GAUGE & COLOUR", THEME_COLOR_ACCENT_TEAL);

    lv_obj_t *ecu = settings_add_dropdown(sec, "ECU Preset:", "Custom\nMaxxECU\nHaltech\nFord BA/BF/FG", 0);
    lv_obj_add_event_cb(ecu, rpm_ecu_dropdown_event_cb, LV_EVENT_VALUE_CHANGED, NULL);

    lv_obj_t *gm = settings_add_dropdown(sec, "Gauge Max:", RPM_OPTS, 0);
    lv_dropdown_set_selected(gm, (rpm_gauge_max >= 3000 && rpm_gauge_max <= 12000)
                                 ? (rpm_gauge_max - 3000) / 200 : 0);
    lv_obj_add_event_cb(gm, rpm_gauge_roller_event_cb, LV_EVENT_VALUE_CHANGED, NULL);

    lv_obj_t *rl = settings_add_dropdown(sec, "Redline RPM:", RPM_OPTS, 0);
    lv_dropdown_set_selected(rl, (rpm_redline_value >= 3000 && rpm_redline_value <= 12000)
                                 ? (rpm_redline_value - 3000) / 200 : 0);
    lv_obj_add_event_cb(rl, rpm_redline_roller_event_cb, LV_EVENT_VALUE_CHANGED, NULL);

    lv_obj_t *rc = settings_add_dropdown(sec, "RPM Colour:", RPM_COLOR_OPTS, 0);
    lv_dropdown_set_selected(rc, rpm_color_idx(values_config[idx].rpm_bar_color));
    lv_obj_add_event_cb(rc, rpm_color_dropdown_event_cb, LV_EVENT_VALUE_CHANGED, NULL);

    /* ── Limiter ── */
    settings_section_t *lim = settings_add_section(tab, "LIMITER", THEME_COLOR_ACCENT_AMBER);

    lv_obj_t *le = settings_add_dropdown(lim, "Effect:",
        "None\nBar Flash\nBar & Circles Flash\nCircles Flash\nBar Solid\nBar & Circles Solid\nCircles Solid", 0);
    uint8_t eff = values_config[idx].rpm_limiter_effect;
    lv_dropdown_set_selected(le, eff < 8 ? ((uint8_t[]){0,0,1,2,3,4,5,6})[eff] : 0);
    lv_obj_add_event_cb(le, rpm_limiter_effect_dropdown_event_cb, LV_EVENT_VALUE_CHANGED, NULL);

    lv_obj_t *lr = settings_add_dropdown(lim, "Limiter RPM:", RPM_OPTS, 0);
    int32_t lv = values_config[idx].rpm_limiter_value;
    lv_dropdown_set_selected(lr, (lv >= 3000 && lv <= 12000) ? (lv - 3000) / 200 : 0);
    lv_obj_add_event_cb(lr, rpm_limiter_roller_event_cb, LV_EVENT_VALUE_CHANGED, NULL);

    lv_obj_t *lc = settings_add_dropdown(lim, "Limiter Colour:", RPM_COLOR_OPTS, 0);
    lv_dropdown_set_selected(lc, rpm_color_idx(values_config[idx].rpm_limiter_color));
    lv_obj_add_event_cb(lc, rpm_limiter_color_dropdown_event_cb, LV_EVENT_VALUE_CHANGED, NULL);

    /* ── Lighting & Background ── */
    settings_section_t *light = settings_add_section(tab, "LIGHTING", THEME_COLOR_ACCENT);

    lv_obj_t *lights = settings_add_switch(light, "RPM Lights:", values_config[idx].rpm_lights_enabled);
    lv_obj_add_event_cb(lights, rpm_lights_switch_event_cb, LV_EVENT_VALUE_CHANGED, NULL);

    bool bg_on = values_config[idx].rpm_background_enabled;
    lv_obj_t *bgsw = settings_add_switch(light, "RPM Background:", bg_on);
    lv_obj_add_event_cb(bgsw, rpm_background_switch_event_cb, LV_EVENT_VALUE_CHANGED, NULL);
    ...
    if (!bg_on) lv_obj_add_flag(bg_box, LV_OBJ_FLAG_HIDDEN);
    lv_obj_add_event_cb(bgsw, warnings_toggle_cb, LV_EVENT_VALUE_CHANGED, bg_box);

    settings_section_t *bgs = bg_box;
    lv_obj_t *bgc = settings_add_dropdown(bgs, "Background Colour:", RPM_COLOR_OPTS, 0);
    lv_dropdown_set_selected(bgc, rpm_color_idx(values_config[idx].rpm_background_color));
    lv_obj_add_event_cb(bgc, rpm_background_color_dropdown_event_cb, LV_EVENT_VALUE_CHANGED, NULL);

    lv_obj_t *bgt = settings_add_dropdown(bgs, "Background RPM:", RPM_OPTS, 0);
    int32_t bv = values_config[idx].rpm_background_value;
    lv_dropdown_set_selected(bgt, (bv >= 3000 && bv <= 12000) ? (bv - 3000) / 200 : 0);
    lv_obj_add_event_cb(bgt, rpm_background_threshold_roller_event_cb, LV_EVENT_VALUE_CHANGED, NULL);
}
...
{
    uint8_t idx = value_id - 1;
    settings_section_t *sec = settings_add_section(tab, "SPEED OPTIONS", THEME_COLOR_ACCENT_TEAL);
    lv_obj_t *ud = settings_add_dropdown(sec, "Speed Units:", "KMH\nMPH", 0);
    lv_dropdown_set_selected(ud, values_config[idx].use_mph ? 1 : 0);
    lv_obj_add_event_cb(ud, speed_units_dropdown_event_cb, LV_EVENT_VALUE_CHANGED, NULL);
}
...
typedef struct { lv_obj_t *custom_btn; lv_obj_t *ratio_btn; } gear_dd_ctx_t;
...
static void set_btn_vis(lv_obj_t *btn, bool show)
{
    ...
    if (show) lv_obj_clear_flag(btn, LV_OBJ_FLAG_HIDDEN);
...
static void tire_circ_input_cb(lv_event_t *e)
{
    ...
    if (code != LV_EVENT_VALUE_CHANGED && code != LV_EVENT_DEFOCUSED) return;
...
static void ratio_input_cb(lv_event_t *e)
{
    ...
    if (idx < 0) values_config[GEAR_VALUE_ID - 1].reverse_gear_ratio = val;
    else         values_config[GEAR_VALUE_ID - 1].gear_ratios[idx]    = val;
}
...
{
    if (lv_event_get_code(e) != LV_EVENT_CLICKED) return;
    open_custom_gear_overlay(values_config[GEAR_VALUE_ID - 1].gear_detection_mode);
}
                                   ...
                                   float value, int user_idx)
{
    ...
    char buf[20];
    if (value > 0.0001f) snprintf(buf, sizeof(buf), "%.3f", value);
    else                 buf[0] = '\0';
...
static void open_speed_rpm_overlay(void)
{
    ...
    settings_section_t *vs = settings_add_section(scroll, "VEHICLE SETUP", THEME_COLOR_ACCENT_TEAL);

    char buf[20];
    snprintf(buf, sizeof(buf), "%.1f", cfg->tire_circumference_mm);
    lv_obj_t *tc = settings_add_text_input(vs, "Tire Circ. (mm):", "e.g. 1980", buf);
    ...
    lv_obj_t *fd = settings_add_text_input(vs, "Final Drive Ratio:", "e.g. 3.420", buf);
    ...
    /* ── Gear Ratios section ── */
    settings_section_t *gr = settings_add_section(scroll, "GEAR RATIOS", THEME_COLOR_ACCENT_AMBER);
...
static void build_display_tab_gear(lv_obj_t *tab)
{
    ...
    settings_section_t *sec = settings_add_section(tab, "GEAR DETECTION", THEME_COLOR_ACCENT_TEAL);

    lv_obj_t *dd = settings_add_dropdown(sec, "Detection Mode:",
        "Custom\nMaxxECU\nHaltech\nFord\nSpeed/RPM Ratio", 0);
    lv_dropdown_set_selected(dd, gear_mode);

    /* "Edit Custom Values" button — visible only for Custom mode (0) */
    lv_obj_t *cbtn = make_gear_action_btn(tab, LV_SYMBOL_EDIT "  EDIT CUSTOM GEAR VALUES");
    if (gear_mode != 0) lv_obj_add_flag(cbtn, LV_OBJ_FLAG_HIDDEN);
    lv_obj_add_event_cb(cbtn, gear_custom_btn_cb, LV_EVENT_CLICKED, NULL);

    /* "Edit Speed/RPM Ratios" button — visible only for Speed/RPM Ratio mode (4) */
    lv_obj_t *rbtn = make_gear_action_btn(tab, LV_SYMBOL_SETTINGS "  EDIT SPEED/RPM RATIOS");
    if (gear_mode != 4) lv_obj_add_flag(rbtn, LV_OBJ_FLAG_HIDDEN);
...
void config_modal_open(lv_obj_t *screen, uint8_t value_id)
{
    ...
    if (is_panel) {
        ...
        pctx->value_id = value_id; pctx->val_lbl = pval;
        lv_timer_t *ptimer = lv_timer_create(panel_preview_timer_cb, 500, pctx);
        lv_obj_add_event_cb(prev_pane, prev_timer_del_cb, LV_EVENT_DELETE, ptimer);
    ...
    /* ── Bar preview ────────────────────────────────────────────────────── */
        ...
        lv_obj_set_style_bg_color(pbar, THEME_COLOR_PANEL, LV_PART_MAIN | LV_STATE_DEFAULT);
        lv_obj_set_style_radius(pbar, THEME_RADIUS_SMALL, LV_PART_MAIN | LV_STATE_DEFAULT);
        lv_obj_set_style_bg_color(pbar, values_config[idx].bar_in_range_color,
                                   LV_PART_INDICATOR | LV_STATE_DEFAULT);
        lv_obj_set_style_radius(pbar, THEME_RADIUS_SMALL, LV_PART_INDICATOR | LV_STATE_DEFAULT);
    ...
    /* ── RPM preview ─────────────────────────────────────────────────────── */
        ...
        menu_rpm_value_label = rval;   /* existing update callbacks keep this live */
    ...
    char title[48];
    if      (is_bar)   snprintf(title, sizeof(title), "BAR %d SETTINGS",   bar_idx + 1);
    else if (is_rpm)   snprintf(title, sizeof(title), "RPM SETTINGS");
    else if (is_speed) snprintf(title, sizeof(title), "SPEED SETTINGS");
    else if (is_gear)  snprintf(title, sizeof(title), "GEAR SETTINGS");
    else               snprintf(title, sizeof(title), "PANEL %d SETTINGS", value_id);
    ...
    lv_obj_set_style_bg_color(preset_btn, THEME_COLOR_ACCENT_DIM, LV_STATE_PRESSED);
    ...
    { uint8_t *p = id_alloc(value_id);
      ...
      lv_obj_add_event_cb(preset_btn, free_value_id_event_cb, LV_EVENT_DELETE, p); }
    ...
    lv_obj_set_style_bg_color(tab_btns, THEME_COLOR_ACCENT_DIM, LV_PART_ITEMS | LV_STATE_CHECKED);
    lv_obj_set_style_bg_opa(tab_btns, LV_OPA_COVER, LV_PART_ITEMS | LV_STATE_CHECKED);
    lv_obj_set_style_text_color(tab_btns, THEME_COLOR_TEXT_PRIMARY, LV_PART_ITEMS | LV_STATE_CHECKED);
    lv_obj_set_style_border_side(tab_btns, LV_BORDER_SIDE_BOTTOM, LV_PART_ITEMS | LV_STATE_CHECKED);
    lv_obj_set_style_border_color(tab_btns, THEME_COLOR_ACCENT, LV_PART_ITEMS | LV_STATE_CHECKED);
    ...
    if      (is_bar)   build_display_tab_bar(tab_display, value_id);
    else if (is_rpm)   build_display_tab_rpm(tab_display, value_id);
    else if (is_speed) build_display_tab_speed(tab_display, value_id);
    else if (is_gear)  build_display_tab_gear(tab_display);
    else               build_display_tab_panel(tab_display, value_id);

    if (is_bar) build_alerts_tab_bar(tab_alerts, value_id);
    else        build_alerts_tab_panel(tab_alerts, value_id);
