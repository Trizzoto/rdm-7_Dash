...
static void delete_old_screen_cb(lv_timer_t *t) {
    ...
    if (s) { destroy_preconfig_menu(); lv_obj_del(s); }
}
...
static void clear_menu_refs(void) {
    ...
    for (int i = 0; i < 8; i++)
        menu_panel_value_labels[i] = menu_panel_boxes[i] = menu_panel_labels[i] = NULL;
    for (int i = 0; i < 2; i++) menu_bar_objects[i] = menu_bar_labels[i] = NULL;
}

static void do_screen_transition(lv_obj_t *old, lv_obj_t *btn) {
    ui_Screen3 = lv_obj_create(NULL);
    if (!ui_Screen3) { lv_obj_clear_state(btn, LV_STATE_DISABLED); return; }
...
static lv_obj_t *make_panel_lbl(lv_obj_t *box, const char *txt,
    ...
    lv_obj_set_width(l, w); lv_label_set_long_mode(l, LV_LABEL_LONG_CLIP);
    lv_obj_set_x(l, 0); lv_obj_set_y(l, y); lv_obj_set_align(l, LV_ALIGN_CENTER);
    return l;
}
...
void create_menu_objects(lv_obj_t *parent, uint8_t value_id)
{
    ...
    const char *cur = (ui_Value[idx] && lv_obj_is_valid(ui_Value[idx])) ? lv_label_get_text(ui_Value[idx])
                    : (strlen(previous_values[idx]) > 0) ? previous_values[idx] : "0";
    menu_panel_value_labels[idx] = make_panel_lbl(menu_panel_boxes[idx],
        cur, THEME_FONT_DASH_VALUE, 9, 140);
