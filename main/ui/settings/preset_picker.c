...
void show_preconfig_menu(lv_obj_t * parent)
{
    ...
    // Trigger ECU dropdown event to populate version dropdown with first ECU's versions
...
typedef struct {
    ...
    lv_obj_t *hi_ver;
    lv_obj_t *hi_sig;
    preset_apply_cb_t  apply_cb;      /* NULL → use config_bridge */
    void              *apply_cb_ctx;
} picker_st_t;
...
static void _open_picker_overlay(uint8_t value_id,
                                  preset_apply_cb_t cb, void *cb_ctx)
{
    ...
    lv_obj_t *brand_list = make_col(body, "BRAND",    true,  260, 356);
    st->ver_list          = make_col(body, "PROTOCOL", true,  260, 356);
    st->sig_list          = make_col(body, "CHANNEL",  false, 260, 356);
...
{
    (void)parent_screen;
    _open_picker_overlay(value_id, NULL, NULL);
}

void open_preset_picker_with_cb(preset_apply_cb_t cb, void *ctx)
{
    _open_picker_overlay(0, cb, ctx);
}
