...
void value_long_press_event_cb(lv_event_t *e)
{
    ...
    if (now - last_long_press_time < 500) return;
...
static void menu_button_hide_timer_cb(lv_timer_t *timer)
{
    ...
    if (menu_button_hide_timer) { lv_timer_del(menu_button_hide_timer); menu_button_hide_timer = NULL; }
}
...
void screen3_touch_event_cb(lv_event_t *e)
{
    ...
    if (code == LV_EVENT_PRESSED) {
        ...
        if (dur < 300 && ui_Menu_Button && lv_obj_is_valid(ui_Menu_Button)) {
            ...
            if (menu_button_hide_timer) lv_timer_del(menu_button_hide_timer);
...
static void setup_menu_close_btn_cb(lv_event_t *e)
{
    ...
    if (ui_Screen3 && lv_obj_is_valid(ui_Screen3)) {
        ...
        if (scr && lv_obj_is_valid(scr)) { lv_obj_del_async(scr); ui_Setup_Menu_Screen = NULL; }
    }
}
...
static void menu_button_clicked_cb(lv_event_t *e)
{
    ...
    lv_obj_set_size(cb, 100, 50); lv_obj_align(cb, LV_ALIGN_BOTTOM_MID, 0, -20);
    lv_obj_set_style_bg_color(cb, THEME_COLOR_BTN_CANCEL, LV_PART_MAIN|LV_STATE_DEFAULT);
    lv_obj_set_style_radius(cb, 10, LV_PART_MAIN|LV_STATE_DEFAULT);
    lv_obj_t *cl = lv_label_create(cb); lv_label_set_text(cl, "CLOSE"); lv_obj_center(cl);
...
void ui_Screen3_screen_init(void)
{
    ...
    lv_obj_set_style_bg_color(ui_Screen3, THEME_COLOR_BG, LV_PART_MAIN|LV_STATE_DEFAULT);
    ...
    for (int i = 0; i < 13; i++) {
        ...
        if (i < 8) { ui_Box[i] = ui_CustomText[i] = NULL; }
        memset(previous_values[i], 0, sizeof(previous_values[i]));
    }
    rpm_bar_gauge = NULL;
    reset_can_tracking = true;

    /* Create all widget layers */
    widget_panel_create(ui_Screen3);
    widget_rpm_bar_create(ui_Screen3);
    widget_speed_create(ui_Screen3);
    widget_gear_create(ui_Screen3);
    widget_bar_create(ui_Screen3);
    widget_indicator_create(ui_Screen3);
    widget_warning_create(ui_Screen3);
    ...
    lv_obj_set_style_bg_opa(ui_Menu_Button, 60, LV_PART_MAIN|LV_STATE_DEFAULT);
    ...
    lv_obj_set_style_text_color(ml, THEME_COLOR_TEXT_PRIMARY, LV_PART_MAIN|LV_STATE_DEFAULT);
    lv_obj_center(ml);
    ...
    lv_obj_set_x(ui_RDM_Logo_Text, 0); lv_obj_set_y(ui_RDM_Logo_Text, -65);
    lv_obj_set_align(ui_RDM_Logo_Text, LV_ALIGN_CENTER);
    lv_obj_add_flag(ui_RDM_Logo_Text, LV_OBJ_FLAG_ADV_HITTEST|LV_OBJ_FLAG_CLICKABLE);
