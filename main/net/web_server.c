...
/* ── /api/gear/config — configure the CALCULATED_GEAR synthetic signal ──
 * GET  → returns current config (wheel circ, final drive, ratios[], enabled)
 * POST → updates config + persists to NVS; takes effect on the next tick.
 *
 * Body format (POST):
 *   {
 ...
 *     "ratios": [0.0, 3.321, 1.902, 1.308, 1.000, 0.759],
 *     "enabled": true
 *   }
 * ratios[0] is reserved for Neutral (0.0); ratios[1..N] are 1..N gears. */
