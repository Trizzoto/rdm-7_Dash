...
void speed_rpm_gear_update_timer_cb(lv_timer_t *timer) {
	...
	if (!rpm_valid || !speed_valid || current_rpm <= 500 ||
		current_speed < 5.0f) {
	...
	if (tire_circ_mm <= 0 || final_drive <= 0) {
		ESP_LOGW("GEAR", "Speed/RPM Ratio mode enabled but tire/final drive "
						 "not configured!");
		return;
	}

	// Calculate current overall gear ratio
	// Formula: gear_ratio = (RPM * tire_circumference_mm * 60) / (speed_kph *
	// final_drive_ratio * 1000000) This simplifies to: gear_ratio = (RPM *
	// tire_circ_mm * 0.00006) / (speed_kph * final_drive)
	float current_ratio =
		(current_rpm * tire_circ_mm * 0.00006f) / (speed_kmh * final_drive);

	// Find the closest matching gear by comparing ratios
	// Check reverse first (negative speed indicates reverse, but we'll check
	// ratio anyway)
	int best_gear = -1;
	float smallest_diff = 999999.0f;
	static int last_gear = -1; // Remember last gear for hysteresis (-1 =
	...
	for (int i = 0; i < 10; i++) {
		...
		// Allow 25% tolerance for ratio matching (increased from 15% for better
		// stability)
		float tolerance = gear_ratio * 0.25f;

		// Add hysteresis: if this was the previous gear, give it preference
		// (reduce its diff by 20%)
		if (i + 1 == last_gear) {
			diff *= 0.8f;
		}
	...
	if (log_counter++ % 10 ==
		0) { // Log every 10th calculation (every 2 seconds)
		ESP_LOGI(
			"GEAR",
			"Speed/RPM Ratio: RPM=%d, Speed=%.1f km/h, Ratio=%.3f, Gear=%d",
...
void gear_ecu_dropdown_event_cb(lv_event_t *e) {
	...
	// Indices: 0 = Custom, 1 = MaxxECU, 2 = Haltech, 3 = Ford, 4 = Speed/RPM
	// Ratio
...
void create_speed_rpm_ratio_config_menu(void) {
	...
	lv_label_set_text(instructions,
					  "Configure tire size, final drive, and gear "
	...
	lv_obj_align(speed_rpm_tire_circumference_input, LV_ALIGN_TOP_LEFT, 250,
				 65);
	lv_obj_add_style(speed_rpm_tire_circumference_input, get_common_style(),
					 LV_PART_MAIN);
	...
	lv_obj_add_event_cb(speed_rpm_tire_circumference_input, keyboard_event_cb,
						LV_EVENT_ALL, NULL);
	...
	lv_obj_add_style(speed_rpm_final_drive_input, get_common_style(),
					 LV_PART_MAIN);
	lv_obj_clear_flag(speed_rpm_final_drive_input,
					  LV_OBJ_FLAG_SCROLLABLE); // Remove scrolling
	...
	lv_obj_add_event_cb(speed_rpm_final_drive_input,
						...
						NULL);
	lv_obj_add_event_cb(speed_rpm_final_drive_input, keyboard_event_cb,
						LV_EVENT_ALL, NULL);

	// Gear Ratios Grid (Reverse + 1-10, total 11 inputs)
	const char *gear_labels[] = {"Reverse", "1st", "2nd", "3rd", "4th", "5th",
	...
	for (int i = 0; i < 11; i++) {
		...
		lv_label_set_text(gear_label, gear_labels[i]);
		lv_obj_set_style_text_color(gear_label, THEME_COLOR_TEXT_PRIMARY, 0);
		lv_obj_align(gear_label, LV_ALIGN_TOP_LEFT, 50 + col * 150,
					 120 + row * 50);

		// Gear ratio input
		if (i == 0) {
			// Reverse gear input
			speed_rpm_reverse_ratio_input =
				lv_textarea_create(speed_rpm_screen);
			...
			lv_obj_align(speed_rpm_reverse_ratio_input, LV_ALIGN_TOP_LEFT,
						 90 + col * 150, 115 + row * 50);
			lv_obj_add_style(speed_rpm_reverse_ratio_input, get_common_style(),
							 LV_PART_MAIN);
			lv_obj_clear_flag(speed_rpm_reverse_ratio_input,
							  LV_OBJ_FLAG_SCROLLABLE); // Remove scrolling
			...
			// Add event callbacks
			lv_obj_add_event_cb(speed_rpm_reverse_ratio_input,
								speed_rpm_reverse_ratio_event_cb,
								LV_EVENT_VALUE_CHANGED, NULL);
			lv_obj_add_event_cb(speed_rpm_reverse_ratio_input,
								keyboard_event_cb, LV_EVENT_ALL, NULL);
		} else {
			// Forward gear inputs (1-10)
			int gear_index =
				i - 1; // Map to gear_ratios array (0-9 for gears 1-10)
			speed_rpm_gear_ratio_inputs[gear_index] =
				lv_textarea_create(speed_rpm_screen);
			lv_textarea_set_one_line(speed_rpm_gear_ratio_inputs[gear_index],
									 true);
			lv_obj_set_width(speed_rpm_gear_ratio_inputs[gear_index], 80);
			lv_obj_set_height(speed_rpm_gear_ratio_inputs[gear_index], 30);
			lv_obj_align(speed_rpm_gear_ratio_inputs[gear_index],
						 LV_ALIGN_TOP_LEFT, 90 + col * 150, 115 + row * 50);
			lv_obj_add_style(speed_rpm_gear_ratio_inputs[gear_index],
							 get_common_style(), LV_PART_MAIN);
			lv_obj_clear_flag(speed_rpm_gear_ratio_inputs[gear_index],
							  LV_OBJ_FLAG_SCROLLABLE); // Remove scrolling
			...
			// Add event callbacks
			lv_obj_add_event_cb(speed_rpm_gear_ratio_inputs[gear_index],
								speed_rpm_gear_ratio_event_cb,
								LV_EVENT_VALUE_CHANGED, NULL);
			lv_obj_add_event_cb(speed_rpm_gear_ratio_inputs[gear_index],
								keyboard_event_cb, LV_EVENT_ALL, NULL);
		}
	...
	lv_label_set_text(back_label, "Back");
	lv_obj_center(back_label);
	lv_obj_add_event_cb(back_btn, speed_rpm_ratio_back_btn_event_cb,
						LV_EVENT_CLICKED, NULL);
	...
	lv_label_set_text(save_label, "Save");
	lv_obj_center(save_label);
	lv_obj_add_event_cb(save_btn, speed_rpm_ratio_save_btn_event_cb,
						LV_EVENT_CLICKED, NULL);
...
void widget_gear_create(lv_obj_t *parent) {
	...
	lv_obj_set_style_bg_color(ui_Gear_Panel, THEME_COLOR_BG,
							  LV_PART_MAIN | LV_STATE_DEFAULT);
...
uint64_t *widget_gear_get_last_can_time(void) {
	return &last_gear_can_received;
}

/* ── Phase 2: widget_t factory ───────────────────────────────────────────── */

static void _gear_on_signal(float value, bool is_stale, void *user_data) {
	(void)user_data;
	if (is_stale) {
		update_gear_ui_immediate("-", 0);
		return;
	}
	...
	update_gear_ui_immediate(buf, raw);
}
...
static void _gear_open_settings(widget_t *w) { (void)w; }
static void _gear_to_json(widget_t *w, cJSON *out) {
...
static void _gear_from_json(widget_t *w, cJSON *in) {
	...
	if (gd->signal_name[0] != '\0')
		gd->signal_index = signal_find_by_name(gd->signal_name);
}
static void _gear_destroy(widget_t *w) {
	free(w->type_data);
	free(w);
}
...
widget_t *widget_gear_create_instance(void) {
	...
	w->from_json = _gear_from_json;
	w->destroy = _gear_destroy;
