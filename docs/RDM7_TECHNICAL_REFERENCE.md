...
| `GET /api/presets` | Flat array of the 318-row built-in preconfig catalog: `{ecu, version, label, can_id(hex string), endianess, bit_start, bit_length, scale, offset, decimals, is_signed}`. |
...
| MaxxECU `1.3` | 0x520, Intel | Covers 1.2 and 1.3 — 1.3 is a strict superset (same ids/bits/scales, extra frames only), so the separate 1.2 preset was merged away 2026-09-12; `ecu_preset_find()` still resolves a saved `"1.2"` onto this entry. Full set incl. oil temp/pressure (0x536) and fuel pressure (0x537) |
...
| Link ECU `AiM Stream` | 0x5F0 (tuner-set, `stream_span` 0x0F), Intel | Sixteen plain consecutive ids — **not** multiplexed, unlike the Generic Dash. Full-scale 16-bit, so divisors look odd (TPS = raw/650, temps = raw/190 − 45). Pressures converted to kPa on the way in (AiM sends mBar for MAP/baro, bar for oil/fuel/boost). FUEL_LEVEL deliberately unbound (AiM sends litres, the slot is a %). **Decode is from the AiM spec only — not yet confirmed against a car.** |
