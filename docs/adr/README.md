...
| [0055](0055-a-device-on-the-wrong-bitrate-is-invisible-not-broken.md) | Accepted | A device on the wrong bit rate is invisible, not broken — on a CAN bus wrong-speed, unplugged and dead are one observation, so a tool that asks a human to interpret silence costs them the afternoon it cost here. The dash becomes the CAN interface (guarded raw TX, tracker reset, promiscuous) and `/api/can/config` finally APPLIES the rate instead of only saving it; Studio's keypad wizard hunts every rate and both protocols, writes one request at a time because overlapping SDOs are silently dropped, and re-finds the device after anything that could have moved it — a protocol switch resets it to the OTHER protocol's factory rate | `main/net/web_server_can.c`, `main/net/web_server_system.c`, `../rdm7-desktop/src/tauri-overlay.html` (`kpw*`), `../rdm7-desktop/tools/check_keypad.js` |
| [0056](0056-a-page-that-needs-a-progress-bar-is-more-than-one-page.md) | Accepted | A page that needs a progress bar is more than one page — the keypad workspace ran a five-step strip along the bottom explaining what order to read the one screen in, and repeated a five-paragraph ECU explainer under every key. It is four sections in the lap timer's shell now (DESIGN/LIVE/CONNECTION/ECU), theme variables re-bound rather than every rule restyled, and each removed paragraph either became a tooltip on its own control or a labelled table row said once | `../rdm7-desktop/src/tauri-overlay.html` (`kpb-*`, `kpSetView`, `kpRenderConnection`, `kpRenderEcuPage`), `../rdm7-desktop/tools/check_keypad.js` |
| [0057](0057-the-editor-wears-the-brand-from-outside.md) | Accepted | The editor wears the brand from outside — the dash editor is the firmware's own page (ADR-0007 forbids editing it here) and has no root element, so the three-band shell is built at RUNTIME by the overlay and the header's controls are MOVED into it, not copied: they are wired by id and inline onclick, so a copy would leave every caller pointing at a hidden header. Re-parenting by id fails SILENTLY on a re-sync, which is the one reason check_dash.js exists | `../rdm7-desktop/src/tauri-overlay.html` (`dsb-*`, `_dsbBuild`, `_dsbSync`), `../rdm7-desktop/tools/check_dash.js` |
...
Strictly sequential. The next ADR is `0079` (`0077` is claimed by the CAN emulation work, `can_emu.h`). Don't reuse numbers, even if a draft is abandoned — leave a stub if needed (`0011-abandoned.md` with one line of explanation).
